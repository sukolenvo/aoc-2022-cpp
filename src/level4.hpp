...
auto parsePairs(const std::string_view& input)
{
  ...
  const auto parsePair = [](auto line, auto beginIndex, auto endIndex) {
    ...
    if (separator == std::string_view::npos) { throw std::invalid_argument("can't find separator"); }
    int rangeStart{};
    auto status = std::from_chars(line.data() + beginIndex, line.data()  + separator, rangeStart);
    if (status.ec != std::errc{}) { throw std::invalid_argument(line.data()); }
    int rangeEnd{};
  ...
  for (const auto& line: lines) {
    ...
    if (comma == std::string_view::npos) { throw std::invalid_argument("can't find comma"); }
    pairs.emplace_back(parsePair(line, 0, comma),
      parsePair(line, comma + 1, line.length()));
  }
  return pairs;
}
