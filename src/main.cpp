...
int main(int argc,
  const char **
    argv)// NOLINT(bugprone-exception-escape) check is unreliable on Windows, see:
         // https://stackoverflow.com/questions/61014184/clang-tidys-bugprone-exception-escape-behaves-weirdly-with-msvc-stl
{
  try {
    const std::array levels{
      ...
      std::function(&level19::run),
      std::function(&level20::run),
      std::function(&level21::run),
      std::function(&level22::run),
      std::function(&level23::run),
      std::function(&level24::run),
      std::function(&level25::run),
    };
