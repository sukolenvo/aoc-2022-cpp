...
auto part1(const auto &input)
{
  ...
  const auto zeroPos = static_cast<size_t>(
    std::distance(items.begin(), std::find(items.begin(), items.end(), std::pair<int, bool>{ 0, false })));
...
struct Item
{
  int id;
  long long value;
  explicit Item(auto value_) : id(itemCounter++), value(value_)
  {}
};
...
auto part2(const auto &input)
{
  ...
  items.reserve(lines.size());
  std::transform(lines.begin(), lines.end(), std::back_inserter(items), [](const auto &line) {
    return Item{ static_cast<long long>(readNumber(line.begin(), line.end())) * 811589153L };
  });
  const auto runShuffle = [&] {
    for (int i = 0; i < static_cast<int>(items.size()); ++i) {
      auto itemIterator = std::find_if(items.begin(), items.end(), [&](const auto &item) { return item.id == i; });
      if (itemIterator->value == 0) {
        continue;
      }
      const auto oldPos = std::distance(items.begin(), itemIterator);
      int pos = static_cast<int>(
        (itemIterator->value + static_cast<long long>(oldPos)) % static_cast<long long>(items.size() - 1));
      if (pos < 0) {
        pos += static_cast<int>(items.size() - 1);
      }
  ...
  const auto zeroPos = static_cast<size_t>(std::distance(
    items.begin(), std::find_if(items.begin(), items.end(), [](const auto &item) { return item.value == 0; })));
  const auto fPos = (zeroPos + 1000) % items.size();
  const auto sPos = (zeroPos + 2000) % items.size();
  const auto tPos = (zeroPos + 3000) % items.size();
